...
LinearizedMeasurement gate_measurement(const LinearizedMeasurement& m,
  ...
  // Precompute P H^T (18 x m) so each innovation variance H_i P H_i^T is a
  // single row-column dot product.
  const Eigen::Matrix<double, 18, Eigen::Dynamic> PHt = P * m.H.transpose();
  ...
  out.H = Eigen::Matrix<double, Eigen::Dynamic, 18>(
      static_cast<Eigen::Index>(keep.size()), 18);
